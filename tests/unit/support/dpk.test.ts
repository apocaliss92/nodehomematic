...
describe('support/dpk', () => {
  it('makeDpk crea una DataPointKey con i campi forniti', () => {
  ...
  it('dpkToUniqueId produce una stringa minuscola', () => {
    const dpk = makeDpk('MyCCU-HmIP-RF', 'VCU0000001:1', 'VALUES', 'STATE');
    expect(dpkToUniqueId(dpk)).toBe('myccu-hmip-rf:vcu0000001:1:values:state');
  });

  it('round-trip con channelAddress che contiene un colon', () => {
  ...
  it('round-trip con channelAddress senza colon (device-level)', () => {
  ...
  it('uniqueIdToDpk è esattamente l inverso di dpkToUniqueId (lowercased)', () => {
    const ids = [
      'iface-1:vcu0000001:1:values:state',
      'iface-2:abc123:master:aes_active',
      'a:b:1:c:d', // channelAddress con colon = "b:1"
    ];
    for (const id of ids) {
